...
import { CreateUserDto, ListUserDto, ListUserLeaderBoardDto, UpdateUserScoreDto } from './dto/user.dto';
...
export class UsersService {
  ...
  async updateUserScore(updateUserScoreDto: UpdateUserScoreDto): Promise<User> {
    ...
    // Push monthly user's score to redis using sorted data type
    this.redisClient.zadd(
      `user:leaderboard:${new Date().getMonth()}`,
      'INCR',
      score,
      `user:${updatedUser['_doc']._id}`,
    );
   ...
   * @param listUserDto
   */
  async leaderBoard(listUserDto: ListUserLeaderBoardDto) {
    const page = listUserDto.page || 1;
    const perPage = listUserDto.perPage || 10;
    ...
    const leaderboard = await this.redisClient.zrevrange(
      `user:leaderboard:${listUserDto.type === 'monthly' ? new Date().getMonth() : ''}`,
      min,
      max,
      'WITHSCORES',
    );
